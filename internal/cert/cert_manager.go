...
// CertManager manages TLS certificates for the sharing service
...
func NewCertManager(ctx *bootstrap.Context) (*CertManager, error) {
	l := ctx.NewLoggerHelper("sharing/cert")

	caCertPath := os.Getenv("SHARING_CA_CERT_PATH")
	if caCertPath == "" {
		caCertPath = "/app/certs/ca/ca.crt"
	}
	serverCertPath := os.Getenv("SHARING_SERVER_CERT_PATH")
	if serverCertPath == "" {
		serverCertPath = "/app/certs/server/server.crt"
	}
	serverKeyPath := os.Getenv("SHARING_SERVER_KEY_PATH")
	if serverKeyPath == "" {
