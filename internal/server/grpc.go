...
func NewGRPCServer(
	...
	if certManager != nil && certManager.IsTLSEnabled() {
		ms = append(ms, mtls.MTLSMiddleware(
			...
			// warden-service covers warden's server-cert fallback when it lacks a client cert.
			mtls.WithAllowedIdentities("lcm-admin", "lcm-backup", "lcm-warden", "warden-service"),
		))
	}
