...
// the schema-agnostic sqldump engine over sharing's own tables (from ent
// migrate.Tables). Replaces the legacy per-field sharing.service.v1.BackupService,
...
func NewSqlBackupService(ctx *bootstrap.Context) *SqlBackupService {
	...
	return &SqlBackupService{
		log:    ctx.NewLoggerHelper("sharing/service/sql-backup"),
		engine: sqldump.New(dsn, sqldump.Options{Module: "sharing", Tables: tables}),
	}
}
...
func (s *SqlBackupService) ImportBackup(stream commonV1.BackupService_ImportBackupServer) error {
	...
	if opts == nil {
		return errors.New("sharing: first ImportBackup message must carry options")
	}
	...
	if err != nil {
		...
		return stream.SendAndClose(&commonV1.ImportBackupResponse{Success: false, Module: "sharing", Warnings: []string{err.Error()}})
	}
	out := &commonV1.ImportBackupResponse{Success: true, Module: "sharing", Warnings: res.Warnings}
